...
export default class Acomodacao {
    ...
    public toString(): string {
        return `Nomenclatura: ${this.nomeAcomadacao}\n` +
            `-- Quantidade de leitos para solteiros: ${this.camaSolteiro}\n` +
            `-- Quantidade de leitos para casais: ${this.camaCasal}\n` +
            `-- Climatização: ${this.climatizacao ? 'sim' : 'não'}\n` +
            `-- Quantidade de garagens disponíveis: ${this.garagem}\n` +
            `-- Quantidade de suites: ${this.suite}`;
    }
}